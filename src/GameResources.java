...
public final class GameResources extends Canvas implements Runnable {
   ...
   // OOP Helper wrapper for clean modding/editing
   ...
   private void l_void() {
            ...
            protected final void paint(Graphics var1) {
               ...
               if (offscreenImage == null) {
                  offscreenImage = Image.createImage(320, 240);
                  offscreenGraphics = offscreenImage.getGraphics();
               }

               // Render actual gameplay elements to 320x240 buffer
               if (this.v_byte == 1) {
                  a_bool6(offscreenGraphics, a_obj_ImageArr[this.c_byteArr[1]], 0, 0, 320, this.A_int, this.a_short - 0, this.b_short - 0 + this.ba - this.ax_byte, 0);
                  ...
                  a_bool6(offscreenGraphics, a_obj_ImageArr[M_byte], 0, 0, 320, 240, this.a_short - 0, this.b_short - 0 + 0, 0);
                  if (this.s_bool && !this.J_bool && this.aN < 0) {
                     offscreenGraphics.setColor(0);
                     b_void8(offscreenGraphics, this.a_short - 0, this.b_short - 0 + 0, 320, 16 + -this.aN);
                     b_void8(offscreenGraphics, this.a_short - 0, this.b_short - 0 + 16 + 240 - -this.aN + 0, 320, -this.aN);
                  }
               }

               // Scale offscreen buffer to fit client window dimensions
               GameCanvas.drawScaledImage(var1, offscreenImage, 0, 0, screenW, screenH);
               this.z_bool = false;
            }
   ...
   private void i_void2(int var1) {
      ...
      try {
         do {
            ...
            // Ép int (& 0xFF) trước khi trừ để tránh tràn số byte signed
            var6 = 0 + ((this.e_byteArr[5252 + var1] & 0xFF) - (this.I_byte & 0xFF)) * 22 + 11 + this.e_byteArr[5454 + var1];
            var7 = 0
               + ((this.e_byteArr[5353 + var1] & 0xFF) - (this.K_byte & 0xFF)) * 16
               + 8
      ...
      try {
         ...
         for (this.ag_int = 0; this.ag_int < this.P_byte + 3; this.ag_int++) {
            ...
            if ((var7 = this.ag_int + this.K_byte) < this.d_short) {
               ...
               // Row nằm ngoài bản đồ (ag_int + K_byte >= d_short): xóa g_byteArr2
               // để tránh stale data tạo circular chain → infinite loop trong i_void2()
               for (int __af = this.O_byte - 1; __af >= 0; __af--) {
                  this.g_byteArr2[this.ag_int][__af] = 0;
               }
            }
         }
   ...
   private void c_void3(byte var1) {
      try {
         this.a_obj_ImageArr2[2][0] = Image.createImage("/pi8/image_" + var1 + ".png");
      } catch (Exception var9) {
         System.err.println("Failed to load /pi8/image_" + var1 + ".png");
      }
   }
